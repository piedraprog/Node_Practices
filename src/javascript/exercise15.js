/**
 * Ejercicio de recortar una oracion y agregarle 3 puntos al final dependiendo de la longitud
 * 
 */
...
const transform = (value, wordsToCut = null) => {
  if (value) {
    ...
    if(value.split('').length > 15) {
      ...
      console.log(title[1].split('').length >= 7)

        const newTitleLength = Math.floor(title.length / 2);
        const newTitle =  title[1].split('').length >= 8 ? title.slice(0, newTitleLength) : title.slice(0, 2);
